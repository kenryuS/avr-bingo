...
U80& U80::operator=(const U80& other) {
    if (this != &other) {
        for (int i = 0; i < 5; i++) {
            val[i] = other.val[i];
        }
...
bool U80::operator[](const size_t n) {
    return (val[4 - (n / 16)] & ((uint16_t)1 << (n % 16))) != 0;
}
...
void U80::toggle(const size_t n) {
    uint8_t idx = 4 - (n / 16);
    uint8_t s = n % 16;
